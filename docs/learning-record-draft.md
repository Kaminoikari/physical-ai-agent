...
L2 為什麼只設計 `execute` 和 `query` 兩個動詞，而不是把每個技能各開一個方法？因為動詞數量一旦跟技能數量綁定，每加一個技能就要動介面，L3 也得跟著改——這會把解耦講回頭路。把「做什麼」收斂成 `execute(skill, params)`、把「現在如何」收斂成 `query(question)`，技能集的成長就只發生在 L1，介面本身保持封閉。L3 看到的永遠是同一份合約，這也是為什麼換底層不必改上層。
...
這些重試與中止路徑都由單元測試固定下來，外部依賴（Claude、模擬器）全部 mock，所以判定不依賴單次 rollout 的運氣，每次跑結果一致。這套行為在 mock 與真模擬兩種後端各跑一遍，共六個情境（單步成功、多步排序、拒絕、澄清、長程失敗中止、狀態查詢），兩種後端表現一致——正是解耦要拿出的證據：同樣的編排邏輯，換了底層仍守住同樣的行為界線。

這裡有一個容易被忽略的取捨：把規劃放到 LLM，代價是每一步決策都要過一次模型推論，比起端到端 VLA 多了延遲與 token 成本。我接受這個代價，因為這層要的是「對」而不是「快」——順序錯誤、做了不該做的動作，在真機上是會撞壞東西的失敗，值得用一次推論換取可審查、可測試的決策。而真正吃時間的逐幀控制仍留在 policy，沒有被 LLM 接管。
...
重構正確性的關鍵是 parity：加速不能改壞成敗判定。這裡刻意保留舊的 subprocess 路徑而不是直接刪掉——它成了重構的真值來源。比對方法是把每個任務的 `pc_success` 以同一門檻（50%）二值化成「成功/失敗」，再逐任務對照兩個引擎的結果；用二值化而非比連續分數，是因為兩條路徑的隨機性（環境初始狀態、取樣）本來就會讓分數有細微差異，真正要守的是「成敗判定」這個下游會用到的結論一致，而不是數字逐位相同。所以我寫了對照 bench，在同一門檻下逐任務比對兩個引擎的成功/失敗，兩邊完全一致才採用。實際在 Kaggle GPU 上跑時還抓到一個只在真硬體才會現形的 bug——`PreTrainedConfig.type` 是唯讀 property，舊程式碼手動賦值它，但這條路徑被 lazy import 包住，Mac 上的 mock 測試（57 個全綠）涵蓋不到，只有真的載模型才會觸發。修掉後實測穩態約 2× 加速，parity 全部一致。這也說明了為什麼 mock 測試綠了還不能算數：介面邊界以外的整合行為，終究得在真環境驗一次。

**範圍界線。** 全程是模擬加真資料，沒有做實體手臂的 closed-loop 評估——真機上「policy 輸出動作 → 手臂執行 → 相機回授 → 下一步」這個閉環尚未跨越，文件中明確標註，不含糊帶過。差距具體在哪：模擬裡的觀測是乾淨的、動力學是確定的，真機則有相機標定、延遲、夾爪打滑、光照變化這些 sim-to-real gap，微調過的 policy 在真硬體上的成功率還沒被量過。要把這段補上需要的是實體 SO-101、一套安全的評估場景、以及真機 rollout 的紀錄管線——這些是下一階段的事，現在不假裝已經有。整條走過的路徑：編排層解耦 → 真資料微調 VLA → 執行管線優化，對應的是這個領域實際的工作流順序，而不是只挑一個點做深。
