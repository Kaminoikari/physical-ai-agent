...
rollout**，專測更難的語言推理——語意分群（「把**所有醬料**都收」→ execute(2,3,4,5)）、排除否定
（「**除了**番茄醬…」）、3+ 物件、排序約束。真 Claude log 還揭露了「番茄醬」這類**真歧義**
（繁中口語偏指 ketchup，但選單同時有 ketchup 與 tomato sauce）——正是該觸發澄清、policy 碰不到的
語言坑。`python demo_plan.py "<指令>"` 本機即可跑，細節見 demo-results.md 情境⑥。
