...
**實測（Kaggle T4 x2、`libero_object`、task 0,1、SmolVLA `HuggingFaceVLA/smolvla_libero`）：**

| task | in-process | subprocess | 成敗 parity |
|---|---|---|---|
| 0（冷啟，含一次性載 policy）| 244.8s | 267.9s | ✅ 都 100% |
| 1（in-process 已暖機、複用 policy）| **131.5s** | 272.3s | ✅ 都 100% |
| 2 task 總計 | **376.3s** | 540.2s | — |

- **穩態加速**：暖機後每 task 131.5s vs subprocess 每 task ~270s → **約 2.0×**，每 task 省 ~139s（即被消掉的「重載 policy」開銷）。
- **整體（2 task）**：1.44×；task 數越多越逼近 2×，因為一次性載入被攤提（in-process 只在 task 0 付一次、subprocess 每 task 都付）。
- **誠實界定**：rollout 本身（280 步 SmolVLA 推論）的計算量固定、省不掉；加速全來自「policy 只載一次」。成敗判定邏輯不變（門檻仍套用 `pc_success`），parity 全 ✅ 證明加速未改壞正確性。
> GR00T N1.5 視 Ampere+ GPU 取得情況再測（T4 跑不動，見 spec 附錄 A）。
...
- **慢（已解）**：subprocess 版每次重載 policy（~270s/task）。已實作 `InProcessRolloutEngine`（policy/env
  常駐複用，直接呼叫 `eval_policy()`）解此瓶頸——Kaggle T4 實測穩態約 **2.0× 加速**、每 task 省 ~139s，
  成敗 parity 全 ✅（見 ⑦）。subprocess 版保留為 parity baseline。
